...
export class TripOperationsService implements OnModuleInit, OnModuleDestroy {
  ...
  async listTrips(user: any, query: any) {
    ...
    /*
     * Tự đồng bộ dữ liệu cũ: một số lần thay HDV trước đây chỉ cập nhật
     * guide_assignments nhưng chưa cập nhật trip_operations.guide_id.
     * Vì màn hình Điều hành lọc theo op.guide_id, HDV mới sẽ bị thiếu chuyến.
     * Chỉ đồng bộ các assignment đang còn hiệu lực của chính HDV hiện tại.
     */
    if (gid) {
      await this.prisma.$executeRawUnsafe(
        `UPDATE trip_operations op
  ...
  async incidents(user: any, operationId: number) {
    ...
    return this.prisma.$queryRawUnsafe<any[]>(
      `SELECT it.*,
       ...
       DATE_FORMAT(it.updated_at,'%Y-%m-%d %H:%i:%s') AS updatedAt,
       g.full_name AS reporterName, u.full_name AS assignedAdminName,
  ...
  async editOwnIncident(
    ...
    if (this.role(user) !== "guide") {
      throw new ForbiddenException(
        "Chỉ hướng dẫn viên mới dùng chức năng này.",
      );
    }
    ...
    const rows = await this.prisma.$queryRawUnsafe<any[]>(
      `SELECT
       ...
       WHERE it.id=? AND it.trip_operation_id=?
       LIMIT 1`,
      ticketId,
      operationId,
    );
    ...
    if (Number(rows[0].guideId) !== Number(gid)) {
      throw new ForbiddenException("Bạn chỉ có thể sửa sự cố do mình gửi.");
    }

    if (Number(rows[0].adminReplyCount || 0) > 0) {
      throw new BadRequestException(
        "Admin đã phản hồi sự cố này nên bạn không thể chỉnh sửa nữa.",
      );
    }
    ...
    await this.prisma.$executeRawUnsafe(
      `UPDATE incident_tickets
       SET category=?,
           ...
           location_name=?,
           updated_at=NOW()
       WHERE id=? AND trip_operation_id=? AND reported_by_guide_id=?`,
      body?.category || "other",
      body?.severity || "medium",
      title,
      description,
  ...
  async deleteOwnIncident(user: any, operationId: number, ticketId: number) {
    await this.assertWritableOperation(user, operationId);
    ...
    if (Number(rows[0].guideId) !== Number(gid)) {
      throw new ForbiddenException("Bạn chỉ có thể xóa sự cố do mình gửi.");
    }

    if (Number(rows[0].adminReplyCount || 0) > 0) {
      throw new BadRequestException(
        "Admin đã phản hồi sự cố này nên bạn không thể xóa nữa.",
      );
    }
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      await tx.$executeRawUnsafe(
        `INSERT INTO notifications(
         ...
         SELECT ?,LEFT(?,500),?,'user',b.user_id,1,?,UTC_TIMESTAMP(),UTC_TIMESTAMP()
         FROM bookings b
         WHERE b.departure_id=? AND b.user_id IS NOT NULL
           AND b.booking_status IN ('confirmed','completed','waiting_confirmation')
           AND (? IS NULL OR b.pickup_point_id=?)`,
        title,
        content,
